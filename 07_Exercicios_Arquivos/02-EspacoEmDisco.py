...
usuarios = []
espacos = []
with open('02-Usuarios.txt', 'r') as users:
    for i, v in enumerate(users.readlines()):
        soma += int(v) if i % 2 != 0 else 0
        if i % 2 == 0:
            usuarios.append(v.replace('\n', ''))
        else:
            espacos.append(int(v))
users.close()

with open('02-Relatorios.txt', 'a+') as relat:
    cont = 1
    relat.write("""
ACME Inc.     Uso do espaco em disco pelos usuarios
------------------------------------------------------------------------
Nr.  Usuario        Espaco utilizado     % do uso\n""")
    for u, e in zip(usuarios, espacos):
        relat.write(f'{cont:<3} {u:<15} {e / mb:<10.2f}{"MB":<2} {(e / mb) / (soma / mb) * 100:>15.2f}%\n')
        cont += 1
